...
public:
    ...
    void buscaKesimo(int const &k) const;
...
protected:
    ...
    virtual bool insertar(ClaveValor const& cv, Link & a);
    ...
    void buscaKesimoAux(int const &k, Link a, Link &resultado) const;
...
public:
    ...
    int altura;
    int tam_i;
    TreeNode(ClaveValor e, Link i = nullptr, Link d = nullptr, int alt = 1, int tam_izq = 1)
    : cv(e), iz(i), dr(d), altura(alt), tam_i(tam_izq) {}
};
...
// Tenemos que modificar lo que devuelve insertar para poder distinguir cuando se inserta un nuevo nodo
// o solo si se sobreescribe su valor.

template <typename Clave, typename Valor, typename Comparador>
bool TreeMap<Clave, Valor, Comparador>::insertar(ClaveValor const& cv, Link & a) {
    bool tengoQueInsertar = true;
    
    if (a == nullptr) {
        a = new TreeNode(cv);
    }
    else if (menor(cv.clave, a->cv.clave)) {
        tengoQueInsertar = insertar(cv, a->iz);
        
        // Si tenemos que insertar el nodo incrementamos el tam_i
        if (tengoQueInsertar)
            a->tam_i++;
        ...
        tengoQueInsertar = insertar(cv, a->dr);
        reequilibraIzq(a);
    }
    else { // la clave ya está, se actualiza el valor asociado
        a->cv.valor = cv.valor;
        // Si ya existe el valor no hay que insertar el nodo.
        tengoQueInsertar = false;
        
    }
    
    return tengoQueInsertar;
}

template <typename Clave, typename Valor, typename Comparador>
void TreeMap<Clave, Valor, Comparador>::erase(Clave const& clave, Link & a) {
    if (a != nullptr) {
        // si tenemos que quitar un nodo de la izquierda, disminuimos tam_i.
...
void TreeMap<Clave, Valor, Comparador>::rotaDer(Link & k2) {
    ...
    // Le restamos a k2 el numero de nodos que tiene k1.
    k2->tam_i -= k1->tam_i;
    k2 = k1;
...
void TreeMap<Clave, Valor, Comparador>::rotaIzq(Link & k1) {
    ...
    k1->altura = std::max(altura(k1->iz), altura(k1->dr)) + 1;
    k2->altura = std::max(altura(k2->iz), altura(k2->dr)) + 1;
    // Le incrementamos a k2 el numero de nodos que tiene k1.
    k2->tam_i += k1->tam_i;
    k1 = k2;
}
...
template <typename Clave, typename Valor, typename Comparador>
void TreeMap<Clave, Valor, Comparador>::buscaKesimoAux (int const &k, Link a, Link &resultado) const
{
    if (a != NULL)
    {
        if (k < a->tam_i)
            buscaKesimoAux(k, a->iz,resultado);
        else if( k > a->tam_i)
            buscaKesimoAux(k -a->tam_i,a->dr,resultado);
        else
            resultado = a;
...
template <typename Clave, typename Valor, typename Comparador>
void TreeMap<Clave,Valor,Comparador>::buscaKesimo (int const &k) const
{
    Link resultado = NULL;
    buscaKesimoAux(k,raiz,resultado);
    
    if (resultado == NULL)
        std::cout << "??" << "\n";
    else
        std::cout << resultado->cv.clave << "\n";
}
